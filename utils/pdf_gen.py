...
class QuotePDF(FPDF):
    ...
    def header(self):
        ...
        self.cell(
            0, 12, "QUOTATION",
            border=0, align="C", new_x=XPos.RIGHT, new_y=YPos.TOP,
        )
        if self._company:
    ...
    def footer(self):
        ...
        self.cell(
            0, 10, f"Page {self.page_no()}  |  Generated by TradeAI Pro",
            border=0, align="C", new_x=XPos.RIGHT, new_y=YPos.TOP,
        )
        self.set_text_color(0, 0, 0)
...
def generate_quote_pdf(
    ...
    pdf.set_text_color(107, 114, 128)
    pdf.cell(
        0, 6, f"Date: {datetime.datetime.now().strftime('%Y-%m-%d')}",
        border=0, align="R", new_x=XPos.LMARGIN, new_y=YPos.NEXT,
    )
    pdf.set_text_color(0, 0, 0)
    pdf.ln(2)
    ...
    for w, h in zip(COL, HEADERS):
        pdf.cell(w, 8, h, border=0, align="C", fill=True, new_x=XPos.RIGHT, new_y=YPos.TOP)
    pdf.ln()
    pdf.set_text_color(0, 0, 0)
    ...
    pdf.cell(
        total_label_w, 9, "  GRAND TOTAL",
        border=0, align="L", fill=True, new_x=XPos.RIGHT, new_y=YPos.TOP,
    )
    pdf.cell(
        COL[-1], 9, f"${grand_total:,.2f}",
