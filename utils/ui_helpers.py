...
def show_result(
    ...
    """
    统一渲染生成结果区域。
    - result 是 Generator → 流式渲染，完成后存入 session_state.results
    - result 是 str       → 直接展示
    """
    if not result:
        return
    ...
    if hasattr(result, "__iter__") and hasattr(result, "__next__"):
        ...
        with st.container():
            full_text = st.write_stream(result)
        # 流式完成后存储
    ...
    st.text_area(label, result, height=height, key=f"display_{result_key}")
