...
-----------------
所有 AI Prompt 模板统一管理。
每个函数接收业务参数，返回 (prompt: str, system_prompt: str | None) 元组。
在这里修改 Prompt 不需要改动 ai_client.py 或页面代码。
"""
...
2. 简明介绍产品核心优势（2-3点）
3. 突出差异化卖点，避免泛泛而谈
4. 结尾包含明确的 Call to Action（邀请回复或视频会议）
5. 签名占位使用 [Your Name] / [Your Company]

只输出纯文本邮件正文，不要加 markdown 符号或标题。"""
...
def build_inquiry_prompt(
    ...
    signature = your_name if your_name else "[Your Name]"
...
FOLLOWUP_STAGES: dict[str, str] = {
    "已报价": "报价后跟进，确认客户对报价的看法，温和询问是否有问题或需要调整",
